...
def run_on_data(i, method, files_input_name, verbose):
    ...
    data = pd.read_csv('./data/' + file_input_name, delimiter=',', index_col=False, header=0)
    ...
    if method == "GrangerPW":
        model = cd.GrangerPW(nodes, sig_level=0.05, nlags=2)
        model.infer_from_data(data)
    elif method == "GrangerMV":
        model = cd.GrangerMV(nodes, sig_level=0.05, nlags=1)
        model.infer_from_data(data)
    elif method == "TCDF":
        ...
        model = cd.PCTMI(nodes, sig_level=0.05, nlags=1)
        model.infer_from_data(data)
    elif method == "NBCB_pw":
        model = cd.NBCB(nodes, sig_level=0.05, nlags=1)
        model.infer_from_data(data)
    elif method == "NBCB":
        model = cd.NBCB(nodes, sig_level=0.05, nlags=1, pairwise=False)
        model.infer_from_data(data)
    elif method == "tsFCI":
        model = cd.TsFCI(nodes, sig_level=0.05, nlags=1)
        model.infer_from_data(data)
    elif method == "FCITMI":
        model = cd.FCITMI(nodes, sig_level=0.05, nlags=5)
        model.infer_from_data(data)
    elif method == "VarLiNGAM":
        model = cd.VarLiNGAM(nodes, sig_level=0.05, nlags=1)
        model.infer_from_data(data)
    elif method == "TiMINO":
        ...
        model = cd.Dynotears(nodes, sig_level=0.05, nlags=1)
        model.infer_from_data(data)
    else:
        model = None
    ...
    if save_model:
        nx.write_gpickle(model.ghat, "./experiments/graphs/summary_other_and_self_graphs/fmri/"+method+"_"+str(i))
        nx.write_gpickle(model.oghat, "./experiments/graphs/summary_other_graphs/fmri/"+method+"_"+str(i))
        nx.write_gpickle(model.sghat, "./experiments/graphs/summary_self_graphs/fmri/"+method+"_"+str(i))
    ...
    if not isinstance(model, cd.TemporalGraphicalModel):
        ...
        if save_model:
            nx.write_gpickle(model.tghat, "./experiments/graphs/temporal_graphs/fmri/" + method + "_" + str(i))
        # evaluation temporal
...
if __name__ == "__main__":
    ...
    #files_input_name[0] = path_input+files_input_name[0]
    ...
    with open(
            "./experiments/performance_average/summary_other_performance_average/" + str(method) + "_fmri",
            "w+") as file:
    ...
    with open(
            "./experiments/performance_average/summary_self_performance_average/" + str(method) + "_fmri",
            "w+") as file:
    ...
    if results.shape[1] > 16:
        ...
        with open(
                "./experiments/performance_average/temporal_performance_average/" + str(method) + "_fmri",
                "w+") as file:
        ...
        np.savetxt("./experiments/performance_detail/" + str(method) + "_fmri.csv", results,
